...
export default function IDETabManager() {
  ...
  useEffect(() => {
    const handleHashChange = () => {
      ...
      // Note: #contact, #about, #process will naturally scroll to their respective anchors
    };
  ...
  return (
    /* ── WRAPPER EXTERIOR FLOTANTE (ESTILO LONZO VISUALS) ── */
    <div className="w-full bg-[#0a0a0a] p-2.5 sm:p-5 md:p-8 min-h-screen flex flex-col justify-center overflow-x-clip">
      {/* ── CONTENEDOR DE LA VENTANA PRINCIPAL REDONDEADA ── */}
      <div className="w-full max-w-7xl mx-auto bg-[#1e1e1e] rounded-2xl md:rounded-3xl border border-[#2d2d2d] shadow-2xl overflow-hidden relative flex flex-col my-auto">
        ...
        <header className="sticky top-0 z-40 w-full bg-[#181818] border-b border-[#2a2d2e] select-none">
          ...
          <div className="w-full px-4 py-2.5 flex items-center justify-between text-xs">
            ...
            {/* Right: Clean spacing / version info (sin badge LIVE) */}
            <div className="flex items-center gap-2 text-xs font-mono text-[#677d64]">
              <span className="text-[11px] hidden sm:inline text-[#677d64]">v2.4.0</span>
            </div>
          </div>
        ...
        {/* ── CONTENT SWITCHER & GLOBAL SECTIONS ── */}
        <main className="w-full flex-1 flex flex-col overflow-x-clip bg-[#000000]">
          {/* 1. Vista Principal Superior según Pestaña Activa */}
          {activeTab === 'page.tsx' && (
          ...
          {/* 2. Flujo Global Común: Todas las vistas desembocan naturalmente en About y Contact */}
          <CleanEditorialAbout />
