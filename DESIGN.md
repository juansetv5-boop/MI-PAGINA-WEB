...
| Name | Value | Role |
|------|-------|------|
| Void Black | `#000000` | Page canvas, deepest background layer |
| Ground Iron | `#181818` | Primary button fill, card surfaces on dark sections, base UI surface |
| Carbon Veil | `#212525` | Elevated surfaces, nav background, subtly lighter than ground |
| Circuit Border | `#485346` | Outlined/ghost action border, primary interactive hairline — muted green-tinted gray |
| Phosphor Blue-Black | `#1f2a33` | Secondary hairline borders, separator strokes between dark bands |
| Charcoal Rust | `#231c1c` | Subtle warm-tinted dark accent for code-window inner panels |
| Lime Pulse | `#7fee64` | Primary brand accent — logo mark, hero CTA pill, tag fills, active states, illustration base color |
| Phosphor White | `#ddffdc` | Primary text, heading color, icon strokes, filled button text — the dominant readable surface |
| Mint Frost | `#def0dd` | Card surface tint on light sections, secondary text on dark, soft highlight wash |
| Sage 60 | `#8cab87` | Body copy default — readable muted green-gray for paragraphs on dark canvas |
| Sage 40 | `#677d64` | Muted helper text, secondary descriptions, low-priority body content |
| Moss 70 | `#9cbf93` | Eyebrow labels, category tags above section headings |
| Moss 80 | `#aed2a4` | Lead paragraph text, hero subhead — the brightest muted green for lead copy |
| Fern Link | `#859984` | Inline link text, tertiary navigation links on dark |
| Deep Fern | `#697368` | Small uppercase labels, micro-copy, footer text |
| Pine 15 | `#3e4a3c` | Secondary outlined action border, low-emphasis interactive hairline |
...
| body-sm | 14px | 1.43 | -0.364px |
| body | 16px | 1.5 | -0.352px |
| subheading | 20px | 1.5 | -0.36px |
| heading-sm | 24px | 1.3 | -0.312px |
| heading | 30px | 1.2 | -0.36px |
| heading-lg | 42px | 1.05 | -0.336px |
| display | 64px | 1 | -0.448px |
...
- **Void** (`#000000`) — Deepest page canvas, hero background, full-bleed dark sections
- **Ground** (`#181818`) — Primary UI surface, filled button background, standard card surface on dark
- **Carbon** (`#212525`) — Elevated surface for nav bar, code window interiors, selected states
- **Mint Frost** (`#def0dd`) — Light section canvas (alternate surface), soft card tint
- **Lime Pulse** (`#7fee64`) — Highest-emphasis accent surface — active CTA pill, tag fills
